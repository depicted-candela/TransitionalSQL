...
SELECT 
    e.SALARY, 
    LAG(SALARY, 1) OVER(PARTITION BY DEPARTMENTID ORDER BY HIREDATE) PREVIOUS_SALARAY,
    LEAD(SALARY, 1) OVER(PARTITION BY DEPARTMENTID ORDER BY HIREDATE) NEXT_SALARY
FROM ADVANCED_QUERYING.EMPLOYEES e;
