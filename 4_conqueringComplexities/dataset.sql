...
-- Drop table if it exists (for easy re-running) - Placed before the definitive CREATE TABLE
DROP TABLE IF EXISTS conqueringcomplexities.Orders;

-- Create the conqueringcomplexities.Orders table (Definitive, corrected version)
CREATE TABLE IF NOT EXISTS conqueringcomplexities.Orders (
    OrderID NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    CustomerID NUMBER,
    OrderDate DATE DEFAULT SYSDATE,
    OrderDetailsXML XMLTYPE,
    ShippingAddressJSON JSON,
    OrderStatus VARCHAR2(20) DEFAULT 'Pending', -- Corrected: Simple column name
...
-- Turn off substitution variable prompting for '&'
SET DEFINE OFF;
...
-- Populate conqueringcomplexities.Orders
DECLARE
    vAliceID NUMBER;
    vBobID NUMBER;
    vSuperPhoneID NUMBER;
    vOracleBookID NUMBER;
    vCoffeeMakerID NUMBER;
BEGIN
    ...
    SELECT ProductID INTO vSuperPhoneID FROM conqueringcomplexities.Products WHERE ProductName = 'SuperPhone X';
    SELECT ProductID INTO vOracleBookID FROM conqueringcomplexities.Products WHERE ProductName = 'Oracle Master Guide 23ai';
    SELECT ProductID INTO vCoffeeMakerID FROM conqueringcomplexities.Products WHERE ProductName = 'Smart Coffee Maker';

    INSERT INTO conqueringcomplexities.Orders (CustomerID, OrderDate, OrderDetailsXML, ShippingAddressJSON, OrderStatus, TotalAmount) -- Corrected: OrderStatus
    VALUES (vAliceID, TO_DATE('2023-10-15', 'YYYY-MM-DD'),
        XMLTYPE.CREATEXML(
            '<order id="ORD1001" date="2023-10-15T10:30:00">
                <customer custid="' || vAliceID || '"/>
                <items>
                    <item productid="' || vSuperPhoneID || '" quantity="1">
                        <name>SuperPhone X</name>
                        <unitprice>999.00</unitprice>
                    </item>
                    <item productid="' || vOracleBookID || '" quantity="1">
    ...
    VALUES (vBobID, TO_DATE('2023-10-28', 'YYYY-MM-DD'),
        XMLTYPE.CREATEXML(
            '<order id="ORD1002" date="2023-10-28T14:00:00">
                ...
                <items>
                    <item productid="' || vCoffeeMakerID || '" quantity="1">
    ...
    VALUES (vAliceID, TO_DATE('2023-11-05', 'YYYY-MM-DD'),
        XMLTYPE.CREATEXML(
            '<order id="ORD1003" date="2023-11-05T09:15:00">
                <customer custid="' || vAliceID || '"/>
                <items>
                    <item productid="' || vOracleBookID || '" quantity="2"> <!-- Alice buys another 2 Oracle books -->
                        <name>Oracle Master Guide 23ai</name>
                        <unitprice>79.50</unitprice>
                    </item>
                </items>
...
BEGIN
    ...
    SELECT DepartmentID INTO vSalesDeptID FROM conqueringcomplexities.DepartmentsRelational WHERE DepartmentName = 'Sales & Marketing'; -- This will now work
...
-- Restore substitution variable prompting if needed
SET DEFINE ON;